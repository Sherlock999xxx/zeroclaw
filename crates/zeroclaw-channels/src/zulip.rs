...
pub struct ZulipChannel {
    ...
    /// The alias key under `[channels.zulip.<alias>]` this handle is bound to.
    alias: String,
...
impl ZulipChannel {
    ...
    /// Return the alias under `[channels.zulip.<alias>]` that this channel
    /// handle is bound to.
    ...
    async fn register_queue(&self) -> Result<QueueState> {
        ...
        let queue_id = payload
            .queue_id
            .ok_or_else(|| anyhow::Error::msg("Zulip register response missing queue_id"))?;
    ...
    fn parse_message_event(&self, msg: &ZulipMessage) -> Option<ChannelMessage> {
        ...
        let reply_target = match msg.msg_type.as_str() {
            ...
            _other => {
                // Unsupported message type (e.g. huddle) — skip.
                return None;
            }
        };
...
impl ::zeroclaw_api::attribution::Attributable for ZulipChannel {
    fn role(&self) -> ::zeroclaw_api::attribution::Role {
        ::zeroclaw_api::attribution::Role::Channel(::zeroclaw_api::attribution::ChannelKind::Zulip)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
                ...
                let events = match self.poll_events(&queue).await {
                    ...
                    Err(e) => {
                        ...
                        if msg.contains("BAD_EVENT_QUEUE_ID") {
                            // Zulip expired the queue; re-register via outer loop.
                            *self.queue.lock() = None;
                            break;
                        }
                        // Transient poll error — pause and retry.
