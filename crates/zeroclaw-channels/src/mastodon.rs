...
pub struct MastodonChannel {
    ...
    /// The alias key under `[channels.mastodon.<alias>]` this handle is bound to.
    alias: String,
...
impl MastodonChannel {
    ...
    /// Return the alias under `[channels.mastodon.<alias>]` that this channel
    /// handle is bound to.
    ...
    fn handle_stream_text(&self, text: &str, identity: &BotIdentity) -> Option<ChannelMessage> {
        ...
        let notif: Notification = serde_json::from_str(&envelope.payload).ok()?;
...
impl ::zeroclaw_api::attribution::Attributable for MastodonChannel {
    fn role(&self) -> ::zeroclaw_api::attribution::Role {
        ::zeroclaw_api::attribution::Role::Channel(
            ::zeroclaw_api::attribution::ChannelKind::Mastodon,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
    ...
    async fn listen(&self, tx: tokio::sync::mpsc::Sender<ChannelMessage>) -> Result<()> {
        ...
        loop {
            ...
            if consecutive_failures >= STREAM_RETRY_THRESHOLD {
                // Streaming unavailable after repeated attempts; fall back to
                // polling /api/v1/notifications at poll_interval.
