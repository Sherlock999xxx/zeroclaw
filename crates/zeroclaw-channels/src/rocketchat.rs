...
impl RocketChatChannel {
    ...
    /// Return the alias under `[channels.rocketchat.<alias>]` that this channel
    /// handle is bound to.
...
impl ::zeroclaw_api::attribution::Attributable for RocketChatChannel {
    fn role(&self) -> ::zeroclaw_api::attribution::Role {
        ::zeroclaw_api::attribution::Role::Channel(
            ::zeroclaw_api::attribution::ChannelKind::Rocketchat,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
...
impl Channel for RocketChatChannel {
    ...
    async fn listen(&self, tx: tokio::sync::mpsc::Sender<ChannelMessage>) -> Result<()> {
        // When room_ids is empty the listener has nothing to poll and simply
        // idles on the poll interval.
        loop {
