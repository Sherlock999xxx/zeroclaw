...
//   └──────────────┘ └─────────────────────────────────────────────┘
//
// Section list comes from /api/config/sections (single source of truth).
// Picker items come from /api/config/sections/<key>. Picking POSTs
// /api/config/sections/<key>/items/<picked> which instantiates the entry
...
// fields auto-fetch /api/config/catalog/models for the datalist.
...
export default function Onboard() {
  ...
  const refreshReadiness = useCallback(async () => {
    try {
      ...
      const readyToFinish =
        ...
        firstRunRequiredSectionsReady(onboardingSections);
    ...
    getSections()
      .then((resp) => {
        ...
        getMapKeys("agents")
          ...
          .catch(() => {});
        const firstRun = orderFirstRunSections(
          ordered.filter((s) => FIRST_RUN_SECTION_KEYS.has(s.key)),
        );
  ...
  const navigationSections = useMemo(() => {
    ...
    if (
      FIRST_RUN_SECTION_KEYS.has(activeSection.key) &&
      firstRunSections.length > 0
    ) {
  ...
  const bindSelectionToSelectedAgent = useCallback(
    async (sectionKey: string, fieldsPrefix: string) => {
      ...
      try {
        ...
        console.warn(
          "Failed to bind onboarding selection to selected agent:",
          e,
        );
      }
    },
  ...
  const bindMemoryToSelectedAgent = useCallback(
    async (backend: string) => {
      ...
      try {
        await patchConfig([
          {
            op: "replace",
            path: `agents.${selectedAgentAlias}.memory.backend`,
            value: backend,
          },
        ]);
      } catch (e) {
  ...
  const handlePick = async (item: PickerItem) => {
    ...
    try {
      ...
      if (e instanceof ApiError) {
        setError(
          `Couldn't open ${item.label}: [${e.envelope.code}] ${e.envelope.message}`,
        );
      } else {
        setError(
          `Couldn't open ${item.label}: ${e instanceof Error ? e.message : String(e)}`,
        );
      }
    }
  };
  ...
  const validateAdvance = async (): Promise<boolean> => {
    ...
    if (activeSection.key === "providers.models") {
      if (!picked && !activeSection.ready) {
        return blockAdvance([
          "Choose a model provider, then set its required model and credential/auth.",
        ]);
      }
    ...
    if (activeSection.key === "storage" && !picked && !activeSection.ready) {
      return blockAdvance([
        "Choose a storage backend. SQLite is the safe default for single-node setup.",
      ]);
    }

    if (activeSection.key === "memory" && !picked && !activeSection.ready) {
      return blockAdvance([
        "Choose or confirm the persistent memory backend. SQLite is the default; choose none to disable long-term memory.",
      ]);
    }
  ...
  const advanceSection = async () => {
    ...
    try {
      ...
      try {
        const current = await getProp(COMPLETED_SECTIONS_PATH).catch(() => ({
          value: "[]",
        }));
      ...
      await refreshReadiness();
      const idx = navigationSections.findIndex(
        (s) => s.key === activeSection.key,
      );
  ...
  const finishOnboarding = async () => {
    ...
    try {
      ...
      const readyToFinish =
        !status.needs_onboarding && wizardIssues.length === 0;
      setCanFinish(readyToFinish);
      if (!readyToFinish) {
      ...
      try {
        ...
        // eslint-disable-next-line no-console
        console.warn(
          "Daemon reload failed after onboarding; user can retry from /config:",
          e,
        );
        setApplyIssue(
        ...
        {activeSection && (
          <div className="flex flex-col gap-4 max-w-3xl">
            ...
            {activeSection.key === "agents" && selectedAgentAlias ? (
              <>
                ...
                <FirstRunCompleteActions
                  ...
                  onAdvanced={() => {
                    ...
                    if (advancedSections[0])
                      goToSection(advancedSections[0].key);
                  }}
                />
              </>
              ...
              isDefaultProfileSection(activeSection.key) &&
              !editingProfile ? (
              ...
              <OnboardAliasListView
                ...
                onSelectAlias={(alias) =>
                  openWithAlias(pickedType.item, pickedType.sectionKey, alias)
                }
              />
              ...
              <OnboardOneTierAliasView
                ...
                onSelectAlias={async (alias) => {
                  try {
                    ...
                    setEditingProfile(false);
                    await bindSelectionToSelectedAgent(
                      activeSection.key,
                      resp.fields_prefix,
                    );
                    await refreshReadiness();
...
function breadcrumbDetail(
  ...
  if (picked) {
    const alias =
      picked.fieldsPrefix.split(".").slice(-1)[0] ?? picked.item.label;
...
function firstRunReadinessIssues(sections: SectionInfo[]): string[] {
  ...
  return FIRST_RUN_SECTION_ORDER.filter((key) => !byKey.get(key)?.ready).map(
    (key) => `${labels.get(key) ?? key} is not complete yet.`,
  );
}
...
function sidebarLabel(
  ...
  if (section.key === "agents")
    return selectedAgentAlias ? `Agent: ${selectedAgentAlias}` : "Agent";
  return section.label;
}

function orderFirstRunSections(sections: SectionInfo[]): SectionInfo[] {
  const order = new Map<string, number>(
    FIRST_RUN_SECTION_ORDER.map((key, index) => [key, index]),
  );
  ...
  if (
    ...
    if (providerType && providerAlias) {
      return {
        ...
        value: `${canonicalProviderRefSegment(providerType)}.${providerAlias}`,
      };
    }
  }
...
function providerRefForFieldsPrefix(fieldsPrefix: string): string | null {
  ...
  const providerType = parts[2];
  const providerAlias = parts[3];
  return providerType && providerAlias
    ? `${canonicalProviderRefSegment(providerType)}.${providerAlias}`
    : null;
}

function providerTypeForFieldsPrefix(fieldsPrefix: string): string | null {
  const parts = fieldsPrefix.split(".");
  return parts[0] === "providers" && parts[1] === "models" && parts[2]
    ? parts[2]
    : null;
}

async function hasCustomProviderEndpoint(
  fieldsPrefix: string,
): Promise<boolean> {
...
async function configuredLocalModelProviderStepIssues(): Promise<string[]> {
  ...
  for (const item of picker.items) {
    ...
    const catalog = await getCatalogModels(catalogProviderType).catch(
      () => null,
    );
    if (!(catalog?.local ?? isLocalModelProvider(catalogProviderType)))
      continue;
...
async function modelProviderStepIssues(picked: {
  item: PickerItem;
  fieldsPrefix: string;
}): Promise<string[]> {
  const providerRef =
    providerRefForFieldsPrefix(picked.fieldsPrefix) ?? picked.item.label;
  const providerType = providerTypeForFieldsPrefix(picked.fieldsPrefix);
  const catalogProviderType = providerType
    ? canonicalProviderRefSegment(providerType)
    : null;
  const model = await getProp(`${picked.fieldsPrefix}.model`).catch(() => null);
  const modelName = hasTextValue(model?.value)
  ...
  const apiKey = await getProp(`${picked.fieldsPrefix}.api-key`).catch(
    () => null,
  );
  const openAiAuth = await getProp(
    `${picked.fieldsPrefix}.requires-openai-auth`,
  ).catch(() => null);
...
function formTitleFor(
  sectionKey: string,
  picked: { item: PickerItem; fieldsPrefix: string },
): string {
  const alias =
    picked.fieldsPrefix.split(".").slice(-1)[0] ?? picked.item.label;
...
function guideFor(
  sectionKey: string,
  prefix: string,
): { title: string; body: string; items?: string[] } | null {
  if (prefix.startsWith("providers.models.")) {
    ...
    return {
      ...
      items: local
        ? [
            "Required before chat: model and reachable local endpoint or CLI.",
            "Optional: timeout, temperature, request-format, and pricing fields.",
          ]
        : [
            "Required before chat: model and credential/auth.",
            "Optional: timeout, temperature, request-format, and pricing fields.",
          ],
    };
  }
...
const AgentFirstRunForm = forwardRef<
  FieldFormHandle,
  {
    prefix: string;
    title: string;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div>
        ...
        >
          {showAdvanced
            ? "Hide advanced agent settings"
            : "Show advanced agent settings"}
        </button>
      </div>
      {showAdvanced && (
        <FieldForm
    ...
    <div className="flex flex-col gap-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {RISK_PRESETS.map((preset) => (
            ...
            >
              ...
              >
                <span>
                  {savingPreset === preset.key ? "Applying…" : preset.label}
                </span>
                {appliedPreset === preset.key && (
                  <span
                    className="text-xs"
      ...
      {isRisk && appliedPreset && (
        <p className="text-sm" style={{ color: "var(--color-status-success)" }}>
          {RISK_PRESETS.find((preset) => preset.key === appliedPreset)?.label ??
            "Preset"}{" "}
          applied to {alias}.
        </p>
      )}
...
const RISK_PRESETS: RiskPreset[] = [
  {
    ...
    label: "Read-only",
    description:
      "Inspection-oriented: shell commands are limited and medium-risk actions still need approval.",
    level: "readonly",
  ...
  },
  {
    ...
    description:
      "Good first-run posture: common development tools, approval for medium risk, high-risk commands blocked.",
    level: "supervised",
    allowedCommands: [
      "git",
      "npm",
  ...
  },
  {
    ...
    label: "Local dev",
    description:
      "More permissive for a trusted local workspace, while still blocking high-risk command patterns.",
    level: "full",
    allowedCommands: [
      "git",
      "gh",
...
function riskPresetOps(prefix: string, preset: RiskPreset) {
  return [
    ...
    {
      op: "replace" as const,
      path: `${prefix}.allowed-commands`,
      value: preset.allowedCommands,
    },
    {
      op: "replace" as const,
...
function typedMapPathSegment(sectionKey: string, typeKey: string): string {
  return sectionKey.startsWith("providers.")
    ? typeKey.replace(/_/g, "-")
    : typeKey;
}
